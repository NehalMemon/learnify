...
import { updateUserStatus } from '@/app/actions/userAdminActions';
...
export default function AdminUsersPage() {
  ...
  // 1. Explicitly defined Select query with error logging
  ...
  const handleSaveModalChanges = async (
    ...
    setIsSaving(true);
    try {
      const res = await updateUserStatus(userId, changes.status);
      if (res.success) {
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Manage student status, roles, credits, and account access.
          </p>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {users.map((user) => (
                <tr key={user.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-4">
                    <span className={`inline-flex rounded-full px-3 py-1 text-xs font-bold ${roleClasses[user.role]}`}>
                      {user.role}
                    </span>
                  </td>

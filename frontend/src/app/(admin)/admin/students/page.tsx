...
import { Search, UserCog, ChevronLeft, ChevronRight, UserCheck, CheckCircle2, XCircle } from 'lucide-react';
  ...
  const fetchStudents = useCallback(async () => {
    ...
    try {
      ...
      if (fetchErr) {
        ...
        setError(`Failed to load students: ${fetchErr.message}`);
        setUsers([]);
        return;
      }
      ...
      const userList: UserRow[] = (data || []).map((u: ApiUser) => ({
        id: u.id,
        fullName: u.full_name || u.fullName || u.email?.split('@')[0] || 'Student',
        email: u.email,
      ...
      setError('Failed to load students. Please retry.');
      setUsers([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSaveModalChanges = async (
    ...
    try {
      ...
      if (statusRes.success && roleRes.success) {
        ...
        toast.error(statusRes.error || roleRes.error || 'Failed to update student');
      }
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">
            Audit, manage, and update student accounts, status, and credit balances.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border border-sky-100 bg-sky-50/50 p-5 shadow-xs">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-sky-700">Total Students</p>
        ...
        <div className="rounded-2xl border border-emerald-100 bg-emerald-50/50 p-5 shadow-xs">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-emerald-700">Active Students</p>
        ...
        <div className="rounded-2xl border border-rose-100 bg-rose-50/50 p-5 shadow-xs">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-rose-700">Inactive Students</p>
      ...
      <div className="flex flex-col gap-3 text-sm text-gray-500 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-xs font-medium">
          Showing {users.length} of {totalCount} total students (Page {page} of {totalPages})
        </p>
